...
public class DaysAndBouquets {
    ...
    private class MinDay {

        public Integer minDay = -1;
    ...
    public int minDays(int[] bloomDays, int numberOfBouquetsRequired, int numberOfFlowerPerBouquetRequired) {
        ...
        Optional<Integer> cat = Optional.empty();
        ...
        for (int i = 0; i < flowers.size(); i++) {
            ...
            if (bouquet.isEmpty()) {
                ...
                if (deltaToPosition == 0 || deltaToPosition == 1 || deltaToPosition == -1) {
                    bouquet.add(flower);
                    helper(minDay, flower, numberOfFlowerPerBouquetRequired, bouquet, unsettledAlreadyBloomedPositionDayFlowerMap);
                    if (bouquet.size() == numberOfFlowerPerBouquetRequired) {
                        totalNumberOfBouquets++;
                        logger.info(String.format("Bouquet created with flower %s. Total bouquets are %s", flower, totalNumberOfBouquets));
                        bouquet.clear();
...
//            }
            if (totalNumberOfBouquets == numberOfBouquetsRequired) {
                minDay.minDay = flowerBloomDay;
                logger.info(String.format("Minimum days to bloom all flowers is %s", minDay.minDay));
                break;
            }
        }
        logger.info(String.format("Seem like no solution is available. Minimum days to bloom all flowers is default %s", minDay.minDay));
        return minDay.minDay;
    }


    private void helper(MinDay minDay, BloomDayFlower lastAddedFlower, int numberOfFlowerPerBouquetRequired, List<BloomDayFlower> bouquetFlowers, Map<Integer, BloomDayFlower> unsettledAlreadyBloomedDayFlowerMap) {
        if (numberOfFlowerPerBouquetRequired == bouquetFlowers.size() || unsettledAlreadyBloomedDayFlowerMap.isEmpty()) {
        } else {
            if (lastAddedFlower==null){

            } else {
            var latestAddedPosition = lastAddedFlower.flowerPosition;
            // evaluate if above
            ...
            if (bloomedOneAbovePosition != null) {
                ...
                if (bouquetFlowers.size() == numberOfFlowerPerBouquetRequired) {
                    ...
                    helper(minDay,bloomedOneAbovePosition, numberOfFlowerPerBouquetRequired, bouquetFlowers, unsettledAlreadyBloomedDayFlowerMap);
                }
            }
            ...
            if (bloomedOneBelowPosition != null) {
                ...
                if (bouquetFlowers.size() == numberOfFlowerPerBouquetRequired) {
                    ...
                    helper(minDay, bloomedOneBelowPosition,numberOfFlowerPerBouquetRequired, bouquetFlowers, unsettledAlreadyBloomedDayFlowerMap);
                }
            }
        }
        }
